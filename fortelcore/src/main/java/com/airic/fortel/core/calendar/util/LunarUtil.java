...
public final class LunarUtil {
	...
	/** 二十四節氣. */
	/*
	 * private String[] LunarHolDayName = { "小寒", "大寒", "立春", "雨水", "驚蟄", "春分", "清明", "谷雨", "立夏", "小滿", "芒種", "夏至",
	...
	/** 從1900到2049年的陰曆信息. */
	private static final long[] LUNAR_INFO = new long[] { 0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554,
	...
	public static Optional<Lunar> solar2Lunar(int year, int month, int day) {
		...
		/*
		 * 求出了農曆年份。 農曆年份是以幹支標識的，以1900年1月31日己亥年開始。 默認1900為己亥年，1901則為辛醜。 但這個1901與公曆的1901不同，如公曆1901年1月還為己亥年，則其陰曆年為1900。
		 */
		lunar.setYear(iYear);
	...
	public static Optional<Lunar> getPrevLunar(int year, int month, int day, boolean isLeap) {
		...
		if (solarDate.isPresent()) {
			DateTime calendarRefDay = new DateTime(solarDate.get().getYear(), solarDate.get().getMonth(),
					solarDate.get().getDay(), 0, 0, 0, DateTimeZone.forID("UTC"));
			DateTime prevSolarDate = calendarRefDay.plusDays(-1);
			Optional<Lunar> prevLunarDate = LunarUtil.solar2Lunar(prevSolarDate.getYear(),
					prevSolarDate.getMonthOfYear(), prevSolarDate.getDayOfMonth());
	...
	public static Optional<Lunar> getNextLunar(int year, int month, int day, boolean isLeap) {
		...
		if (solarDate.isPresent()) {
			...
			Optional<Lunar> nextLunarDate = LunarUtil.solar2Lunar(nextSolarDate.getYear(),
					nextSolarDate.getMonthOfYear(), nextSolarDate.getDayOfMonth());
